...
## Workflow Entrypoint
Begin from the workflow contract, then open the current instruction file for the active step.

## Recommended Prompt
`Follow the Neuroplast workflow from files only. Read neuroplast/WORKFLOW_CONTRACT.md, neuroplast/manifest.yaml, and the current instruction file. Keep behavior aligned with the documented folder contract, write only to approved locations, and preserve changelog and learning updates.`
