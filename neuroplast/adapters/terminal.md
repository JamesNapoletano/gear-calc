...
## Workflow Entrypoint
Read the workflow contract and current instruction file directly from the filesystem, then work step by step through the required artifacts.

## Recommended Prompt
`Operate the Neuroplast workflow from files only. Read neuroplast/WORKFLOW_CONTRACT.md, neuroplast/manifest.yaml, neuroplast/capabilities.yaml, and the current instruction file. Use the filesystem contract as the source of truth, keep changes non-destructive, and record plan, changelog, and learning updates in the required locations.`
