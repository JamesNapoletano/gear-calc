...
export const validate = (inputs) => {
  ...
  if (!isStandardPressureAngle) {
    warnings.push('Spur gears typically use a 14.5° or 20° pressure angle.')
  }
  if (Math.abs(inputs.addendumCoeff - 1) > 0.01) {
    warnings.push('Spur gear addendum is typically 1.0 × module.')
  }
  if (Math.abs(inputs.dedendumCoeff - 1.157) > 0.01) {
    warnings.push('Spur gear dedendum is typically 1.157 × module.')
  }
  return warnings
}
